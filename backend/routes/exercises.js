...
/**
 * @api {get} /exercises/user/:userId Get exercises by user ID
 * @apiName GetExercisesByUserId
 * @apiGroup Exercise
 *
 * @apiParam {Number} userId ID of the user.
 *
 * @apiSuccess {Object[]} exercises List of exercises associated with the user.
 */
router.get('/user/:userId', (req, res, next) => {
  const userId = req.params.userId;
  Exercise.findAll({ where: { userId }, include: [User, ExerciseType, Set] })
    .then((exercises) => {
      res.json(exercises);
    })
    .catch(next);
